// FUNCIIÓN DE VERIFICAR USUARIO - LLAMA LOS ID DEL FORMULARIO DE LOGIN
function VerificarAlumno() {
    var usu = $("#TXT_USUARIO").val();
    var con = $("#TXT_CONTRASENA").val();

    // VALIDACIÓN DE CAMPOS VACÍOS
    if(usu.length==0 || con.length==0){
        return Swal.fire("Mensaje De Advertencia","Llene los campos vacios","warning"); // Mensaje de advertencia.
    }

    // Ajax para la recepción de paramétros.
    $.ajax({
        url:'../controlador/usuario/controlador_verificar_alumno.php',
        type:'POST',
        data:{
            user:usu,
            pass:con
        ...
        if(resp==0){
            ...
            $.ajax({
                ...
                data: {
                    ...
                    idusuario:data[0][0],   // USU_ID_USUARIO
                    user:data[0][1],        // USU_NOMBRE   
                    rol:data[0][14]          // ROL_NOMBRE
                }
            }).done(function(resp){
                let timerInterval
                Swal.fire({
